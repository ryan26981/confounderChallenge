...
**Implementation requirement:** All three figures must be generated with **beautifully formatted, object-oriented matplotlib code** — i.e., create explicit `Figure` and `Axes` objects via `fig, ax = plt.subplots(...)` and call methods on `ax` (e.g., `ax.scatter`, `ax.axhline`, `ax.set_xlabel`, `ax.legend`). Do **not** use the stateful `plt.scatter` / `plt.title` / `plt.xlabel` pyplot API. Code should be clean, well-organized, and readable (logical grouping, consistent spacing, and meaningful variable names, for example, use `dreamy_color = '#4E79A7'` once at the top and reuse it, rather than pasting the hex code into every plotting call).
...
dreamy = patients_randomized_df["doctor_name"] == "Doc Dreamy"
duck = patients_randomized_df["doctor_name"] == "Doc Duck"

mean_dreamy = patients_randomized_df.loc[dreamy, "post_surgical_score"].mean()
mean_duck = patients_randomized_df.loc[duck, "post_surgical_score"].mean()

fig, ax = plt.subplots(figsize=(9, 5), layout="constrained")

ax.scatter(
    patients_randomized_df.loc[dreamy, "patient"],
    patients_randomized_df.loc[dreamy, "post_surgical_score"],
    marker="o",
    s=55,
    facecolors=dreamy_color,
    edgecolors="white",
...
ax.scatter(
    patients_randomized_df.loc[duck, "patient"],
    patients_randomized_df.loc[duck, "post_surgical_score"],
    marker="^",
    s=65,
    facecolors=duck_color,
    edgecolors="white",
...
ax.set_xlabel("Patient Number", fontsize=11)
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=11)
ax.set_title("Post-Surgical Symptom Score (Randomized Assignment)", fontsize=12, pad=10)
ax.grid(True, linestyle=":", linewidth=0.8, alpha=0.65)
ax.legend(frameon=True, loc="lower right", fontsize=10)
...
- **Object-oriented matplotlib:** Use `fig, ax = plt.subplots(...)` and call methods on the `Axes` object (`ax.scatter`, `ax.set_title`, `ax.set_xlabel`, `ax.legend`, etc.). Avoid the stateful `plt.*` API.
- **Beautifully formatted code:** Organize each figure's code into logical blocks (data prep, plotting, styling, annotations), use descriptive variable names, keep line lengths reasonable, and give recurring values (colors, labels, shaded-region bounds) a name at the top of the chunk instead of retyping the raw number or hex code each time.
...
- Test your Python code chunks incrementally as you build each figure
